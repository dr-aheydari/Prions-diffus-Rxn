...
public:
    ...
    tuple <double, double, double > Assign_phi(double x, double y, double z) const
    {
        ...
        else if (hole=='T')
        {
            ...
            {
                phi3 = sqrt(SQR(x-x1_start)+SQR(y-(y1_start*(1+0.15*(t-2.1))))+SQR(z-z1_start))-(r1_start/3);
                //                phi4= phi3/2; //sqrt(SQR(x-x2_start)+SQR(y-(y1_start*(1+0.15*t)))+SQR(z-z2_start))-((r2_start/3)+alpha2*(t));
                //            }
                //
                //            else if (t > 3.2 && t < 7.6)
    ...
    double operator() (double x, double y, double z) const
    {
        ...
        auto [phi1, phi2, phi3] =  Assign_phi(x, y,z);
...
//        my_octree.print_VTK_Format_Cell(zeta_n, "zeta",file_name);
        ...
        n++;
        dt = dt_const * my_octree.dx_finest_resolution();
