...
// prototype functions :
double Mass_Correction_Diff(double initial_mass,double current_mass,double area);
double SoftAdapt_Mass_Correction(double initial_mass,double current_mass,double area);
...
double conversion_rate1 = 0.01 *0;
double mu = 0.2 * 0;
//double D_z = 0.001;
...
public:
    ...
    double operator() (double x, double y, double z) const
    {
        ...
        double phi2 = sqrt(SQR(x-x2_start)+SQR(y-(y1_start*(1+0.35*t)))+SQR(z-z2_start))-(r2_start+alpha2*(t));
...
public:
    ...
    double operator() (double x, double y, double z) const
    {
        ...
        if (phi>0.)
            ...
            return ((ABS(100* exp(-1 * 100 * (SQR(x-x1_start)+SQR(y-y1_start)+SQR(z-z1_start))))));
    }
} psi_initial;
...
public:
    ...
    double operator() (double x, double y, double z) const
    {
        ...
        if (phi>0.)
            ...
            return ((ABS(100 * exp(-1 * 100 * (SQR(x-x1_start)+SQR(y-y1_start)+SQR(z-z1_start)))))); // Gaussian Distribution
...
//typedef struct greaterSmaller SoftAdapt;

SoftAdapt softadapt(double initial_totalMass ,double current_A, double current_B,double num_funcs)
{
    SoftAdapt s;
    double denom = exp(current_A) + exp(current_B);

    double coeff1 = exp(current_A) / (denom) ;
    double coeff2 = exp(current_B) / (denom);
...
#pragma omp parallel for
    ...
    // print to the file for figures
        ofstream filestream("/Users/aliheydari/Documents/Prions Project Data/total_psi_data_diffOnly/A_Diff_React_massCons.txt");
        ofstream filestream2("/Users/aliheydari/Documents/Prions Project Data/total_zeta_data_diffOnly/B_Diff_React_massCons.txt");
...
//        poisson_solver.set_Diagonal_Increment(1.0);
       ...
       if (n == 0)
       {
          ...
          cout <<"Initial 'A' Mass set to : " << total_psi << endl;
       }
       ...
       cout << "corrected mass is : " << corrected_mass << endl;
        ...
        filestream << n << "     " << corrected_mass << endl;
...
//        ls.set_Phi(level_set_n);
        ...
        if (n == 0)
        {
           ...
           cout <<"Initial 'B' Mass set to : " << total_psi << endl;
        }


        double corrected_mass2 = Mass_Correction_Diff(init_mass_B,total_zeta,volume);

        cout << "corrected mass is : " << corrected_mass2 << endl;

        //filestream2 << n << "     " << total_zeta << endl;

        filestream2 << n << "     " << corrected_mass2 << endl;
...
#pragma omp parallel for
        ...
        // print everything
        sprintf(file_name, "/Users/aliheydari/Documents/Prions Project Data/Diff_React_massCons/cell_splitting_%d.vtk", n);
...
double Mass_Correction_Diff(double initial_mass,double current_mass,double volume)
{

    double updated_mass = current_mass +  (initial_mass - current_mass)/(1);
...
//double SoftAdapt_Mass_Correction(double initial_totalMass ,double current_A, double current_B,double num_funcs)
//{
//    double denom = exp(current_A) + exp(current_B);
