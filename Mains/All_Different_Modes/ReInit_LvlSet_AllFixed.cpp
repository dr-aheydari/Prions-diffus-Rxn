...
//void MakeFolder(char hole, char DiffRXN,char DiffOnly,double D_psi, double D_zeta, double gamma_AtoB, double gamma_BtoA, double max_level, double min_level, double tOrder,
//                string* txtPathA,string* txtPathB,string* daughterPathA,string* daughterPathB,string* motherPathB,string* motherPathA);
...
double tOrder = dt;
double t = 3.98; //kind of the same as dt
...
int max_level = 5; //for our adaptive meshing
////////////

// System parameters
double D_psi = 0.00001; // Diffusion coeffcient for "healthy" protein
double D_zeta = 0.00001; // Diffusion coeffcient for aggregate
...
// isNan prototype for throwing exceptions
...
public:
    ...
    double operator() (double x, double y, double z) const
    {
        ...
        {
            phi2 = sqrt(SQR(x-x2_start)+SQR(y-(y1_start*(1+0.35*threshold)))+SQR(z-z2_start))-(r2_start+alpha2*(t));
        ...
        else if (hole=='T')
        {
            ...
            {
                phi3 = sqrt(SQR(x-x1_start)+SQR(y-(y1_start*(1-0.35*(t-threshold))))+SQR(z-z1_start))-(r1_start/5);
            }
